...
public:
    ...
    [[nodiscard]] const constexpr std::wstring& GetTitle() const {
        return title;
    }
    ...
    [[nodiscard]] constexpr std::wstring GetLocation() const {
        return std::wstring(L"Line [") + line + L"] in " + file;
    }

    [[nodiscard]] constexpr std::wstring GetFullMessage() const {
        return GetNote() + L"\nAt: " + GetLocation();
    }
